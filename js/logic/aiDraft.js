 ...
 */
function scoreCandidate(player, roster) {
  ...
  return 0.50 * ratingNorm + 0.28 * popNorm + 0.22 * posNeed;
}
 ...
 * @returns {object|null}
 */
export function chooseAiPick(board, roster) {
  if (!board?.length) return null;
  let best = null;
  let bestScore = -Infinity;
  for (const p of board) {
    const score = scoreCandidate(p, roster);
